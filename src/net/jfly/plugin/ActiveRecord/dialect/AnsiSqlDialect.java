...
import net.jfly.plugin.ActiveRecord.ActiveRecordException;
import net.jfly.plugin.ActiveRecord.CPI;
import net.jfly.plugin.ActiveRecord.DbKit;
import net.jfly.plugin.ActiveRecord.Model;
import net.jfly.plugin.ActiveRecord.ModelBuilder;
import net.jfly.plugin.ActiveRecord.Page;
...
public class AnsiSqlDialect extends Dialect {
	...
	public void forModelUpdate(TableInfo tableInfo, Map<String, Object> attrs, Set<String> modifyFlag, String pKey, Object id, StringBuilder sql, List<Object> paras) {
		...
		for (Entry<String, Object> e : attrs.entrySet()) {
			String colName = e.getKey();
			if (!pKey.equalsIgnoreCase(colName) && modifyFlag.contains(colName) && tableInfo.containsColumnName(colName)) {
				if (paras.size() > 0)
					sql.append(", ");
	...
	public void forDbSave(StringBuilder sql, List<Object> params, String tableName, Record record) {
	...
	public final <T> List<T> buildModel(ResultSet rs, Class<? extends Model> modelClass, int pageSize) throws SQLException, InstantiationException, IllegalAccessException {
		...
		for (int k = 0; k < pageSize && rs.next(); k++) {
			...
			Map<String, Object> attrs = CPI.getAttributeMap(ar);
			for (int i = 1; i <= columnCount; i++) {
				Object value;
				if (types[i] < Types.BLOB)
					value = rs.getObject(i);
